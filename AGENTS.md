...
### Prefer .tscn Files Over Code-Generated UI

**Why:** Scene files (.tscn) are:
- Easier to visualize and edit in the Godot editor
- More maintainable and designer-friendly
- Self-documenting with clear node hierarchy
- Support live editing and previews

**When to use .tscn files (PREFERRED):**
- Static UI layouts (panels, dialogs, menus)
- Editor dock/panel UI
- Toast notifications and popups
- Debug panels and tools
- Any UI that will be instanced once or a few times

**When code generation is acceptable:**
- Spawning many instances of the same item (list items, grid cells)
- Truly dynamic content that changes based on data (achievement list buttons)
- Procedurally generated UI elements

```csharp
// ✅ GOOD - Load UI from scene file
public partial class AchievementDebugPanel : Control
{
    // UI elements defined in .tscn and connected via [Export]
    [Export] private TabContainer TabContainer = null!;
    [Export] private VBoxContainer LocalProviderList = null!;
    ...
    {
        // Clear and rebuild dynamic list items (code generation OK for list items)
        foreach (var child in LocalProviderList.GetChildren())
            child.QueueFree();

        foreach (var achievement in GetAchievements())
        {
            // Instantiate item scene for each entry (spawning many = OK)
            var item = AchievementItemScene.Instantiate<AchievementListItem>();
            item.Setup(achievement);
            LocalProviderList.AddChild(item);
        }
    }
}

// ❌ BAD - Building complex UI in code
public partial class AchievementDebugPanel : Control
{
    public override void _Ready()
    {
        // Don't build static layout in code!
        var tabContainer = new TabContainer();
        AddChild(tabContainer);

        var localTab = new VBoxContainer { Name = "Local" };
        tabContainer.AddChild(localTab);

        var header = new HBoxContainer();
        localTab.AddChild(header);

        var refreshButton = new Button { Text = "Refresh" };
        header.AddChild(refreshButton);
        // ... hundreds more lines of layout code
    }
}
```

### Scene Structure for UI Components

```
MyPanel.tscn                 <- Main scene file (static layout)
├── MarginContainer
│   └── VBoxContainer
...
│       │   └── RefreshButton
│       └── ScrollContainer
│           └── ItemList (VBoxContainer)  <- Dynamic children added here

MyListItem.tscn              <- Reusable item scene (spawned many times)
├── HBoxContainer
│   ├── IconRect
│   ├── NameLabel
│   └── ActionButton
...
### Connecting Scene Nodes to Code

```csharp
// ✅ GOOD - Use [Export] to connect nodes from the scene
public partial class DebugPanel : Control
{
    // These are set in the Godot editor inspector
    [Export] private Button RefreshButton = null!;
    [Export] private VBoxContainer ContentList = null!;
    [Export] private Label StatusLabel = null!;

    public override void _Ready()
    {
        // Nodes are already assigned, just connect signals
        RefreshButton.Pressed += OnRefresh;
    }
}

// ⚠️ ACCEPTABLE - GetNode for nodes defined in same scene
public partial class DebugPanel : Control
{
    ...
    public override void _Ready()
    {
        // Use GetNode when [Export] isn't practical
        _refreshButton = GetNode<Button>("VBox/Header/RefreshButton");
        _refreshButton.Pressed += OnRefresh;
    }
}
```

### PackedScene for Dynamic Instantiation

```csharp
// ✅ GOOD - Use PackedScene for items that will be spawned many times
public partial class AchievementList : Control
{
    [Export] private PackedScene AchievementItemScene = null!;  // Set in editor
    [Export] private VBoxContainer ItemContainer = null!;

    private void PopulateList(Achievement[] achievements)
    {
...
| Scenario | Approach |
|----------|----------|
| Main panel/dialog layout | .tscn file |
| Editor dock UI | .tscn file |
| Debug panel structure | .tscn file |
| Tab container with tabs | .tscn file |
| List of achievements (many items) | Code + PackedScene for items |
| Dynamic button list | Code generation |
| Toast notification template | .tscn file |
| Provider status indicators | .tscn file with [Export] labels |
